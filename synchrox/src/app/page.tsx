    ...
    <div className="donut-chart-container">
      <div className="donut-chart">
        <svg viewBox="0 0 160 160" style={{ transform: 'rotate(-90deg)' }}>
          ...
          <circle cx="80" cy="80" r="70" fill="none" stroke="url(#gradient-ai)" strokeWidth="14"
            strokeDasharray={`${aiDash} ${440 - aiDash}`} strokeDashoffset="0" strokeLinecap="round"
            style={{ transition: 'stroke-dasharray 1s ease' }} />
          <circle cx="80" cy="80" r="70" fill="none" stroke="url(#gradient-human)" strokeWidth="14"
            strokeDasharray={`${humanDash} ${440 - humanDash}`} strokeDashoffset={`${-aiDash}`} strokeLinecap="round"
            style={{ transition: 'stroke-dasharray 1s ease' }} />
          <defs>
...
export default function DashboardPage() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      setStats({
        totalQueries:147, autoApproved:98, pendingReview:12,
        humanApproved:23, humanEdited:11, rejected:3, processing:0,
        avgConfidence:0.76, avgProcessingTime:842, hitlRate:31.3,
        recentQueries:[
          ...
          { id:'3', user_query:'Design microservice architecture', status:'human_edited', confidence_score:0.54, created_at:new Date(Date.now()-600000).toISOString() },
        ],
      });
  ...
  const submitQuery = async () => {
    ...
    try {
      ...
      const data = await res.json();
      const conf = data.query?.confidence_score ?? data.query?.confidenceScore ?? 0;
      setToast(data.query?.status === 'auto_approved'
        ? `✅ Auto-approved (confidence: ${Math.round(conf * 100)}%)`
        : `⚠️ Routed to human review (confidence: ${Math.round(conf * 100)}%)`);
      setQuery('');
      fetchStats();
    } catch { setToast('❌ Failed to submit query'); }
    finally { setSubmitting(false); setTimeout(() => setToast(null), 4000); }
  };
    ...
    <>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:'20px',marginBottom:'32px' }}>
        <div className="glass-card">
          <div style={{ padding:'20px 24px 0' }}><h3 className="section-title">AI vs Human Distribution</h3></div>
          <DonutChart ai={s.autoApproved} human={s.humanApproved + s.humanEdited + s.pendingReview} />
        </div>
        <div className="glass-card" style={{ overflow:'hidden' }}>
          <div style={{ padding:'20px 24px 0' }}><h3 className="section-title">Recent Activity</h3></div>
            ...
            {s.recentQueries.map((q) => (
              <div key={q.id} className="activity-item">
                ...
                <div className="activity-content">
                  <div className="activity-text" style={{ overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:'350px' }}>
                    {q.user_query}
                  </div>
